...
main(int argc, char* argv[])
{
    ...
    std::cout << nNodes << " " << nFlows << " " << nPackets << " " << speed << std::endl;  
    ...
    std::string speedString = "ns3::ConstantRandomVariable[Constant="+ std::to_string(speed) +"]";
    ...
    if(packetSent == 0) delivery_ratio = -1;
    else delivery_ratio = packetReceived / (double)packetSent;
    ...
    if(graph != -1){
        ...
        if(graph == 0){
            ...
            tpfile << speed << "\t" << throughput << endl;
            drfile << speed << "\t" << delivery_ratio << endl;
        }
    }
